...
# Versión: 0.1.1  -- revision previa a la primera ejecucion: se anade cobertura
#              bajo gapto_runtime (positivo, sin contexto y rechazo de D-169),
#              la comprobacion de la FK HMT->movimiento pasa a ser exacta en
#              lugar de "existe alguna FK entre las dos tablas", y se corrige
#              la cabecera a N1..N8.
...
FK_HMT_MOVIMIENTO = "fk_hecho_movimientos_tesoreria__mov"
...
def test_n5_relink_que_rompe_la_conciliacion_destino(esc: Escenario) -> None:
    conc2 = esc.conciliacion(esc.hecho_eur, esc.movimiento, "-100.0000")
    esc.aportacion(esc.hecho_eur, esc.conc, "90.0000")
...
def test_n8_relink_de_conciliacion_a_otra_del_mismo_movimiento(esc: Escenario) -> None:
    conc2 = esc.conciliacion(esc.hecho_eur, esc.movimiento, "-300.0000")
...
def test_fanout_solo_se_validan_las_conciliaciones_comparables(esc: Escenario) -> None:
    ...
    c2 = esc.conciliacion(hecho, esc.movimiento, "-500.0000")     # EUR
...
def test_runtime_sin_contexto_nunca_confirma_la_escritura(esc: Escenario) -> None:
    ...
    esc.db.rollback()
    assert _uno(esc.db, "SELECT count(*) FROM gapto.hecho_aportaciones_pago") == 0
