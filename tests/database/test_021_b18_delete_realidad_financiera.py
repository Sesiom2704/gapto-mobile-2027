...
CON_DELETE = (
    ...
    # Buckets B y C — pendientes de decisión, no se anticipan
    "regla_versiones", "cuenta_participaciones", "entidad_participaciones",
    "usuarios", "cuentas", "documentos",
)
...
def test_b18_matriz_efectiva_de_runtime(db: psycopg.Connection) -> None:
    ...
    assert _cuenta(db, "gapto_runtime", "DELETE") == 50
...
def test_b18_d086_sigue_abierto(db: psycopg.Connection) -> None:
    """Recordatorio ejecutable: 50 tablas conservan DELETE y esperan decisión.

    Si este test empieza a fallar porque el número baja, será porque se han
    aplicado los Buckets B y/o C; en ese momento debe actualizarse aquí y en
    la documentación canónica, no silenciarse.
    """
    assert _cuenta(db, "gapto_runtime", "DELETE") == 50
