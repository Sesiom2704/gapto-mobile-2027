...
#
# Version: 0.1.1
#   0.1.1 (R-08): el discriminante de M20 pasa a ser determinista
#   (test_version_desfasada_en_lifecycle). La carrera C4 se conserva como
#   `corroborante`: se informa si tambien cae, pero no decide el veredicto.
#   Un oraculo concurrente podia reportar DUDOSO sobre un mutante muerto.
# Version: 0.1.0
# ============================================================
...
class Mutante:
    ...
    Tiene que ser DETERMINISTA. Una carrera con hilos no sirve como oraculo:
    su desenlace puede variar entre pasadas y el arnes reportaria DUDOSO sobre
    un mutante realmente muerto. Cuando ademas existe un interleaving que
    cubre la misma invariante, se declara como `corroborante`: se informa si
    tambien cae, pero no decide el veredicto.
    """
...
CONTRACTUALES: list[Mutante] = [
    ...
    Mutante(
        ...
        # Oraculo DETERMINISTA: sin guarda optimista, una version desfasada
        # deja de rechazarse. No usa hilos, asi que cae siempre.
        "test_version_desfasada_en_lifecycle",
        # La carrera sigue existiendo como evidencia de concurrencia, pero ya
        # no decide el veredicto de M20.
...
def correr(lote: list[Mutante]) -> list[tuple[Mutante, str, int, list[str]]]:
    ...
    for mut in lote:
        ...
        nota = ""
        if mut.corroborante:
            cae = any(mut.corroborante in f for f in fallidos)
            nota = f" [corrobora={'si' if cae else 'no'}]"
        print(f"  {mut.ident:4} {estado.split(':')[0]:9} fallos={n:<3} "
              f"{mut.descripcion}{nota}", flush=True)
    return filas
